...
const Chat = ({ toggleChat, chatHistory }: ChatType) => (
  <>
    ...
    <div className="fixed bottom-0 left-1/2 transform -translate-x-1/2 w-full bg-white px-4 rounded-t-2xl shadow-lg z-20 md:max-w-md md:mx-auto overflow-y-scroll max-h-[90vh]">
      <div className="sticky top-0 bg-white p-4 z-30">
...
const ChatInput = () => {
  ...
  const submitChatInput = async (prompt: string) => {
    ...
    try {
      const res = await sendToCohere(prompt, (session.data?.user as any)?.tags);
